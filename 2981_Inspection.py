...
최소 입력 값 보다 큰 입력이 정답에 포함되려면 모두 나머지가 최소 입력 값이 되어야함.
나머지 입력에 최소 입력 값을 뺀 모든 값들의 공약수만이 정답이 될 수 있다는 것이 포인트.
최대공약수를 구해서 최대공약수를 포함한 최대공약수의 약수만을 리스트로 만들어 조건 탐색하면 시간복잡도를 매우 줄일 수 있음.
